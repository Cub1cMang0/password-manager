import handle
from handle import main_work, tkinter, ctk
import sys, os, json, secrets, pyperclip, ctypes
...
def set_icon(window):
    ...
    icon_dir = os.path.join(current_dir, "mini_icons")
    if not os.path.exists(icon_dir):
        os.makedirs(icon_dir)
    png_icon_path = os.path.join(icon_dir, "app_icon.png")
    ico_icon_path = os.path.join(icon_dir, "app_icon.ico") 
    try:
        if not os.path.exists(ico_icon_path):
            ...
            print(f"Generated {ico_icon_path}")
    except ImportError:
        print("Pillow not installed. ICO conversion skipped. Using PNG only.")
    except FileNotFoundError:
        print(f"Original PNG icon not found at {png_icon_path}. Please create it.")
    ...
    if sys.platform.startswith('win'):
        if os.path.exists(ico_icon_path):
            ...
            print("No icon file found for Windows.")
        ...
        if os.path.exists(png_icon_path):
            ...
            print("No PNG icon found for macOS.")
    else:
        if os.path.exists(png_icon_path):
            img = Image.open(png_icon_path)
            photo_image = ImageTk.PhotoImage(img)
            ...
            print("No PNG icon found for Linux/Unix.")
...
def confirm_storage():
    ...
    yes_button, no_button = handle.big_yes_no_buttons(entry_frame, handle_storage)
...
def set_main(generate_rc: bool):
    ...
    question = ctk.CTkLabel(set_m_frame, text=("Before you get started, you need to set up a master password"
                                            ...
                                            , text_color=handle.TEXT_COLOR, font=handle.FONT, wraplength=360)
    question.pack(padx=20, pady=20)
    answer = ctk.CTkEntry(set_m_frame, fg_color=handle.FIRST_COLOR, border_color=handle.THIRD_COLOR, font=handle.FONT, text_color=handle.TEXT_COLOR, width=200)
    answer.pack(padx=20, pady=10)
    def display_recover_key() -> None:
        recover_key = secrets.token_hex(16)
        handle.save_recovery_key(recover_key)
        prompt.deiconify()
            ...
            def twoFA_decision(decision: int):
                ...
                if decision == 1:
                    def post_success():
                        ...
                        if generate_rc:
                            display_recover_key()
                    handle.setup2FA(yes_button, no_button, post_success)
            yes_button, no_button = handle.binary_buttons(set_m_frame, twoFA_decision, "Yes", "No")
...
def main_login() -> None:
    lockout_data = handle.load_lockout_data()
    remaining_time = lockout_data["remaining"]
    attempt = lockout_data["attempts"]
    prompt = ctk.CTkToplevel(app, fg_color=handle.FIRST_COLOR)
    handle.set_toplevel_icon(prompt)
    prompt.title("Login")
    prompt.geometry("720x480")
    if remaining_time > 0:
        ...
        def update_timer(time_left):
            ...
            handle.save_lockout_data(attempt, time_left)
        ...
        recover_button.configure(command=lambda: handle.recover_account(recover_button, prompt, successful_recovery))
    ...
    def master_login_logic(twoFA_enabled: bool):
        ...
        frame.pack(padx=100, pady=100, expand=True, fill="both")
        ...
        def master_login():
            ...
            correct = main_work.check_master(given_password)
            nonlocal attempt
            if correct:
                handle.clear_lockout()
                prompt.destroy()
                ...
                attempt += 1
                if attempt >= 5:
                    handle.save_lockout_data(attempt, 60)
                    prompt.destroy()
                    main_login()
                    return
                else:
                    handle.save_lockout_data(attempt, 0)
    ...
    def login_2FA_logic():
        for child in prompt.winfo_children():
            child.destroy()
        is2FA = main_work.is2FAsetup()
        frame_2FA = ctk.CTkFrame(prompt, fg_color=handle.SECOND_COLOR)
        frame_2FA.pack(padx=100, pady=100, expand=True, fill="both")
        if not is2FA:
        ...
        def authenticate_2FA():
            nonlocal attempt
            successful = handle.check_2FA(twoFA_entry.get())
            if successful:
                handle.clear_lockout()
                prompt.destroy()
                app.deiconify()
                main_work.QUI = handle.retrieve_2FA_key()
...
def confirm_export() -> None:
    ...
    handle.set_toplevel_icon(export_prompt)
    ...
    export_frame.pack(padx=100, pady=100, expand=True, fill="both")
    ...
    def export_decision(decision: int) -> None:      # Gives the user the option to export their passwords.
        if decision == 0:
            ...
            return
        if decision == 1:
            message, file = handle.export_info_enc()
            if file == '':
                export_prompt.destroy()
            elif file == None:
                ...
                ok_button = ctk.CTkButton(export_frame, text="Ok", command= lambda: export_prompt.destroy(), fg_color=handle.FOURTH_COLOR, text_color=handle.TEXT_COLOR, font=handle.FONT, hover=False)
                ok_button.pack(padx=20, pady=20)
    yes_button, no_button = handle.binary_buttons(export_frame, export_decision, "Yes", "No")
...
def confirm_import() -> None:
    ...
    handle.set_toplevel_icon(import_prompt)
    ...
    import_frame.pack(padx=100, pady=100, expand=True, fill="both")
    import_label = ctk.CTkLabel(import_frame, text=("Are you sure you want to import passwords from a file previously exported by this program?"
                                                    "\n (Note: Importing a file not meeting this criteria will not work)"), text_color=handle.TEXT_COLOR, font=handle.FONT, wraplength=360)
        ...
        if decision == 1:
            ...
            def merge_decision(decision: int) -> None:
                ...
                def validate_import() -> None:
                    message, file = handle.import_info(merge, master_entry.get())
                    if file == '':
                        import_prompt.destroy()
                        ...
                        def destroy_export_decision(decision: int) -> None:
                            ...
                            if decision == 0:
                                os.remove(file)
                                import_prompt.destroy()
                        keep_button, delete_button = handle.binary_buttons(import_frame, destroy_export_decision, "Keep", "Delete")
            ...
            merge_button, override_button = handle.binary_buttons(import_frame, merge_decision, "Merge", "Override")
    yes_button, no_button = handle.binary_buttons(import_frame, import_decision, "Yes", "No")
    import_prompt.grab_set()

def confirm_reset() -> None:
    reset_master_prompt = ctk.CTkToplevel(fg_color=handle.FIRST_COLOR)
    handle.set_toplevel_icon(reset_master_prompt)
    ...
    reset_master_frame.pack(padx=100, pady=100, expand=True, fill="both")
        ...
        if decision == 1:
            ...
            if main_work.is2FAsetup():
                ...
                def submit_2FA():
                    ...
                    successful = handle.check_2FA(code_2FA)
                    if successful:
                        reset_master_prompt.destroy()
                        set_main(False)
                    else:
    ...
    yes_button, no_button = handle.binary_buttons(reset_master_frame, reset_master_decision, "Yes", "No")
...
def confirm_disable_2FA() -> None:
    ...
    handle.set_toplevel_icon(disable_2FA_frame)
    ...
    disable_2FA_frame.pack(padx=100, pady=100, expand=True, fill="both")
    disable_2FA_label = ctk.CTkLabel(disable_2FA_frame, text="Are you sure you want to disable 2FA? (Not recommended due to lower security)", text_color=handle.TEXT_COLOR, font=handle.FONT, wraplength=360)
    ...
    def disable_2FA_decision(decision: int):
        ...
        yes_button.pack_forget()
        no_button.pack_forget()
    yes_button, no_button = handle.binary_buttons(disable_2FA_frame, disable_2FA_decision, "Yes", "No")
...
def confirm_enable_2FA() -> None:
    ...
    handle.set_toplevel_icon(enable_2FA_prompt)
    ...
    enable_2FA_frame.pack(padx=100, pady=100, expand=True, fill="both")
    enable_2FA_label = ctk.CTkLabel(enable_2FA_frame, text="Are you sure you want to enable 2FA? (Recommended to increase security)", text_color=handle.TEXT_COLOR, font=handle.FONT, wraplength=360)
    ...
    def enable_2FA_decision(decision: int):
        ...
        if decision == 1:
            enable_2FA_prompt.destroy()
            handle.setup2FA(yes_button, no_button, lambda: update_2FA_status("Enable 2FA"))
    yes_button, no_button = handle.binary_buttons(enable_2FA_frame, enable_2FA_decision, "Yes", "No")
...
def confirm_passyword_wipe() -> None:
    ...
    handle.set_toplevel_icon(wipe_passyword_prompt)
    ...
    wipe_passyword_frame.pack(padx=100, pady=100, expand=True, fill="both")
    twoFA = main_work.is2FAsetup()
    if twoFA:
        ...
        wipe_2FA_entry.pack(padx=20, pady=15)
    else:
    ...
    wipe_master_entry.pack(padx=20, pady=15)
    def wipe_passyword_decision() -> None:
        passyword_success = main_work.check_master(wipe_master_entry.get())
        twoFA_success = handle.check_2FA(wipe_2FA_entry.get()) if twoFA else True
        if passyword_success and twoFA_success:
            wipe_master_entry.pack_forget()
            wipe_2FA_entry.pack_forget() if twoFA else None
            submit_button.pack_forget()
            wipe_passyword_label.configure(text="ARE YOU SURE YOU WANT TO CONTINUE WITH THIS IRREVERSIBLE ACTION OF DELETING ALL PASSWORDS?", text_color=handle.TEXT_COLOR)
            def final_wipe_decision(decision: int) -> None:
                ...
                if decision == 1:
                    ...
                    message = handle.wipe_passywords()
                    ...
                    ok_button.pack(padx=20, pady=20)
            wipe_button, no_wipe_button = handle.binary_buttons(wipe_passyword_frame, final_wipe_decision, "Wipe passwords", "Do not wipe")
        else:
    ...
    for i in range(len(data)):
        try:
            ...
            if search_desc in current_desc.lower():
                ...
                delete_button = ctk.CTkButton(storage_content, text='', image=recycle_img, command=lambda d=data[i]["desc"]: delete_info(handle.fetch(d), d), fg_color=handle.FOURTH_COLOR, hover=False, font=handle.FONT, text_color=handle.TEXT_COLOR, width=0)
...
def update_info(description: str) -> None:
    ...
    handle.set_toplevel_icon(update_prompt)
    ...
    update_frame.pack(padx=100, pady=100, expand=True, fill="both")
    og_passyword = handle.fetch(description)
    ...
    def update_info_decision(decision: int) -> None:
        if decision == 1:
            ...
            handle.delete(og_passyword, og_description)
            handle.store(new_passyword, new_description)
            dump_desc()
            update_prompt.destroy()
        if decision == 0:
            update_prompt.destroy()
    save_button, cancel_button = handle.binary_buttons(update_frame, update_info_decision, "Save", "Cancel")
    save_button.configure(state="disabled", fg_color="gray")
...
def select_theme() -> None:
    ...
    handle.set_toplevel_icon(theme_prompt)
    ...
    theme_frame.pack(padx=100, pady=100, expand=True, fill="both")
    ...
    red_button = ctk.CTkButton(theme_frame, text='', fg_color="#FF0000", command=lambda: handle.color_change("red", app), hover=False, font=handle.FONT, text_color=handle.TEXT_COLOR, width=80)
    red_button.grid(row=1, column=0, padx=10, pady=10)
    red_button.no_theme = True
    orange_button = ctk.CTkButton(theme_frame, text='', fg_color="#FFA500", command=lambda: handle.color_change("orange", app), font=handle.FONT, text_color=handle.TEXT_COLOR, hover=False, width=80)
    orange_button.grid(row=1, column=1, padx=10, pady=10)
    orange_button.no_theme = True
    yellow_button = ctk.CTkButton(theme_frame, text='', fg_color="#FFFF00", command=lambda: handle.color_change("yellow", app), font=handle.FONT, text_color=handle.TEXT_COLOR, hover=False, width=80)
    yellow_button.grid(row=1, column=2, padx=10, pady=10)
    yellow_button.no_theme = True
    green_button = ctk.CTkButton(theme_frame, text='', fg_color="#00FF00", command=lambda: handle.color_change("green", app), font=handle.FONT, text_color=handle.TEXT_COLOR, hover=False, width=80)
    green_button.grid(row=2, column=0, padx=10, pady=10)
    green_button.no_theme = True
    blue_button = ctk.CTkButton(theme_frame, text='', fg_color="#0000FF", command=lambda: handle.color_change("blue", app), font=handle.FONT, text_color=handle.TEXT_COLOR, hover=False, width=80)
    blue_button.grid(row=2, column=1, padx=10, pady=10)
    blue_button.no_theme = True
    purple_button = ctk.CTkButton(theme_frame, text='', fg_color="#800080", command=lambda: handle.color_change("purple", app), font=handle.FONT, text_color=handle.TEXT_COLOR, hover=False, width=80)
    purple_button.grid(row=2, column=2, padx=10, pady=10)
    purple_button.no_theme = True
    black_button = ctk.CTkButton(theme_frame, text='', fg_color="#000000", command=lambda: handle.color_change("black", app), font=handle.FONT, text_color=handle.TEXT_COLOR, hover=False, width=80)
    black_button.grid(row=3, column=0, padx=10, pady=10)
    black_button.no_theme = True
    white_button = ctk.CTkButton(theme_frame, text='', fg_color="#FFFFFF", command=lambda: handle.color_change("white", app), font=handle.FONT, text_color=handle.TEXT_COLOR, hover=False, width=80)
    ...
    font_theme_label = ctk.CTkLabel(theme_frame, text="Click any color to change the program's text color", text_color=handle.TEXT_COLOR, font=handle.FONT)
    font_theme_label.grid(row=4, column=0, columnspan=3, pady=(10, 5))
    black_text_button = ctk.CTkButton(theme_frame, text='', fg_color="#000000", command=lambda: handle.save_text_theme("black", app), font=handle.FONT, hover=False, width=80)
    black_text_button.grid(row=5, column=0, padx=10)
    black_text_button.no_theme = True
    white_text_button = ctk.CTkButton(theme_frame, text='', fg_color="#FFFFFF", command=lambda: handle.save_text_theme("white", app), font=handle.FONT, hover=False, width=80)
    white_text_button.grid(row=5, column=1, padx=10)
    white_text_button.no_theme = True
    custom_text_button = ctk.CTkButton(theme_frame, text='???', command=lambda: handle.customize_text_color(app), text_color=handle.TEXT_COLOR, font=handle.FONT, hover=False, width=80)
    custom_text_button.grid(row=5, column=2, padx=10)
    custom_text_button.no_theme = True
    theme_prompt.grab_set()

def select_font() -> None:
    font_prompt = ctk.CTkToplevel(fg_color=handle.FIRST_COLOR)
    handle.set_toplevel_icon(font_prompt)
    ...
    font_frame.pack(padx=150, pady=40, expand=True, fill="both")
    ...
    font_frame.grid_columnconfigure(1, weight=0)
    font_frame.grid_columnconfigure(2, weight=1)
    font_label.grid(row=0, column=1, pady=(0, 20))
    arial_button = ctk.CTkButton(font_frame, text="Arial", command=lambda: handle.save_font_theme("Arial", app), font=("Arial", 15), text_color=handle.TEXT_COLOR, fg_color=handle.FOURTH_COLOR, hover=False)
    arial_button.grid(row=1, column=1, pady=10)
    arial_button.no_font = True
    times_new_roman_button = ctk.CTkButton(font_frame, text="Times New Roman", command=lambda: handle.save_font_theme("Times New Roman", app), font=("Times New Roman", 15), text_color=handle.TEXT_COLOR, fg_color=handle.FOURTH_COLOR, hover=False)
    times_new_roman_button.grid(row=2, column=1, pady=10)
    times_new_roman_button.no_font = True
    georgia_button = ctk.CTkButton(font_frame, text="Georgia", command=lambda: handle.save_font_theme("Georgia", app), font=("Georgia", 15), text_color=handle.TEXT_COLOR, fg_color=handle.FOURTH_COLOR, hover=False)
    georgia_button.grid(row=3, column=1, pady=10)
    georgia_button.no_font = True
    ms_gothic_button = ctk.CTkButton(font_frame, text="MS Gothic", command=lambda: handle.save_font_theme("MS Gothic", app), font=("MS Gothic", 15), fg_color=handle.FOURTH_COLOR, text_color=handle.TEXT_COLOR, hover=False)
    ms_gothic_button.grid(row=4, column=1, pady=10)
    ms_gothic_button.no_font = True
    calibri_button = ctk.CTkButton(font_frame, text="Calibri", command=lambda: handle.save_font_theme("Calibri", app), font=("Calibri", 15), fg_color=handle.FOURTH_COLOR, text_color=handle.TEXT_COLOR, hover=False)
    calibri_button.grid(row=5, column=1, pady=10)
    calibri_button.no_font = True
    roboto_button = ctk.CTkButton(font_frame, text="Roboto", command=lambda: handle.save_font_theme("Roboto", app), font=("Roboto", 15), fg_color=handle.FOURTH_COLOR, text_color=handle.TEXT_COLOR, hover=False)
    roboto_button.grid(row=6, column=1, pady=10)
    roboto_button.no_font = True
    verdana_button = ctk.CTkButton(font_frame, text="Verdana", command=lambda: handle.save_font_theme("Verdana", app), font=("Verdana", 15), fg_color=handle.FOURTH_COLOR, text_color=handle.TEXT_COLOR, hover=False)
    verdana_button.grid(row=7, column=1, pady=10)
    verdana_button.no_font = True
    segoe_ui_button = ctk.CTkButton(font_frame, text="Segoe UI", command=lambda: handle.save_font_theme("Segoe UI", app), font=("Segoe UI", 15), fg_color=handle.FOURTH_COLOR, text_color=handle.TEXT_COLOR, hover=False)
    segoe_ui_button.grid(row=8, column=1, pady=10)
    segoe_ui_button.no_font = True
    open_sans_button = ctk.CTkButton(font_frame, text="Open Sans", command=lambda: handle.save_font_theme("Open Sans", app), font=("Open Sans", 15), fg_color=handle.FOURTH_COLOR, text_color=handle.TEXT_COLOR, hover=False)
    open_sans_button.grid(row=9, column=1, pady=10)
    open_sans_button.no_font = True
    courier_new_button = ctk.CTkButton(font_frame, text="Courier New", command=lambda: handle.save_font_theme("Courier New", app), font=("Courier New", 15), fg_color=handle.FOURTH_COLOR, text_color=handle.TEXT_COLOR, hover=False)
    courier_new_button.grid(row=9, column=1, pady=10)
...
def customize_colors(previous_prompt) -> None:
    ...
    handle.set_toplevel_icon(custom_theme_prompt)
    ...
    custom_theme_frame.pack(padx=100, pady=100, expand=True, fill="both")
    custom_theme_label = ctk.CTkLabel(custom_theme_frame, text="Create your own theme", font=handle.FONT, text_color=handle.TEXT_COLOR, wraplength=360)
    custom_theme_label.pack(padx=20, pady=10)
    custom_background_button = ctk.CTkButton(custom_theme_frame, text="Background/Text Entries", command=lambda: handle.save_custom_colors("Background/Entry", app), fg_color=handle.FOURTH_COLOR, text_color=handle.TEXT_COLOR, font=handle.FONT, hover=False)
    custom_background_button.pack(padx=20, pady=10)
    custom_frame_button = ctk.CTkButton(custom_theme_frame, text="Frames", command=lambda: handle.save_custom_colors("Frames", app), fg_color=handle.FOURTH_COLOR, text_color=handle.TEXT_COLOR, font=handle.FONT, hover=False)
    custom_frame_button.pack(padx=20, pady=10)
    custom_textbox_button = ctk.CTkButton(custom_theme_frame, text="Output Textboxes", command=lambda: handle.save_custom_colors("Output Text", app), fg_color=handle.FOURTH_COLOR, text_color=handle.TEXT_COLOR, font=handle.FONT, hover=False)
    custom_textbox_button.pack(padx=20, pady=10)
    custom_buttons_button = ctk.CTkButton(custom_theme_frame, text="Buttons", command=lambda: handle.save_custom_colors("Buttons", app), fg_color=handle.FOURTH_COLOR, text_color=handle.TEXT_COLOR, font=handle.FONT, hover=False)
    custom_buttons_button.pack(padx=20, pady=10)
...
generate_pass_length.bind("<KeyRelease>", lambda event: handle.check_strength("Password Length", generate_pass_length, generate_pass_strength))
...
generate_letters_entry.bind("<KeyRelease>", lambda event: handle.check_strength("Letters", generate_letters_entry, letters_entry_strength))
...
generate_numbers_entry.bind("<KeyRelease>", lambda event: handle.check_strength("Numbers", generate_numbers_entry, numbers_entry_strength))
...
generate_symbols_entry.bind("<KeyRelease>", lambda event: handle.check_strength("Symbols", generate_symbols_entry, symbols_entry_strength))
...
generate_output = ctk.CTkTextbox(app, width=generate_frame_w, height=handle.floor((screen_h - generate_frame_h) * 0.13), fg_color= handle.THIRD_COLOR, font=handle.FONT, text_color=handle.TEXT_COLOR)
...
def output_generate_result() -> None:
    ...
    message = handle.generate_passyword(fetch_cb_states(), generate_pass_length)
    if not message == '':
    ...
    for i in range(len(data)):
        try:
            ...
            if len(original_text) > 75:
                ...
                desc_label = ctk.CTkLabel(storage_content, text=shortened_text, text_color=handle.TEXT_COLOR, font=handle.FONT)
                handle.HoverTooltip(desc_label, original_text)
            else:
                desc_label = ctk.CTkLabel(storage_content, text=original_text, text_color=handle.TEXT_COLOR, font=handle.FONT)
            desc_label.grid(row=i, column=0, sticky="w", padx=side_spacing, pady=upper_spacing)
            ...
            update_button.grid(row=i, column=1, sticky="e")
            delete_button = ctk.CTkButton(storage_content, text='', image=recycle_img, command=lambda d=data[i]["desc"]: delete_info(handle.fetch(d), d), fg_color=handle.FOURTH_COLOR, font=handle.FONT, text_color=handle.TEXT_COLOR, hover=False, width=0)
            delete_button.grid(row=i, column=2, sticky="e")
        except KeyError:        # I realized that this would happen if the user enters the app without any passwords stored.
            return
...
def main():
    global app
    set_icon(app)
    first = handle.first_time()
    setup_menu_bar()
    handle.FIRST_COLOR, handle.SECOND_COLOR, handle.THIRD_COLOR, handle.FOURTH_COLOR = handle.load_theme(app)
    app.configure(fg_color=handle.FIRST_COLOR)
    handle.FONT, handle.TEXT_COLOR = handle.load_text_theme(app)
    if first:
        app.withdraw()
        set_main(True)
    else:
